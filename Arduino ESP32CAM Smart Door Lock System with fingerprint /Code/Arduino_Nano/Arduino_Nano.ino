...
void loop() {
  ...
  if (fid == '0' || fid == '1' || fid == '2' || fid == '3' || fid == '4' || fid == '5') {
    ...
    Serial.print("Unknown Face Identified");
    Serial.println(fid);
    lcd.setCursor(0, 0);
    lcd.print(F("Unknown Face    "));
    lcd.setCursor(0, 1);
    lcd.print(F("Identified      "));
    delay(1000);
  }
  ...
  if (flag1 == 1 && v == 0) {
    if (flag2 < 3) {
      ...
      if (flag2 == 3) {
        Serial.println("Exceed Limit   ");
        fid = 'z';
        lcd.setCursor(0, 0);
        lcd.print(F("Exceed FPrint   "));
        lcd.setCursor(0, 1);
        lcd.print(F("Scan Limit      "));
        lcd_msg();
      }
    }
  }
